...
module.exports = function potty_pot(root, remote, command, options)
{
  ...
  self.start = function()
  {
    ...
    function loop()
    {
      ...
      for(var key in _command.env)
      {
        if(typeof _command.env[key] === 'undefined')
          delete process.env[key];
        else
          process.env[key] = _command.env[key];
      }

      __log__('Spawning app.');

      var child = child_process.spawn(_command.command, _command.args, {cwd: _path.resources, detached: true, stdio: ['pipe', 'pipe', 'pipe', 'ipc']});
      ...
      child.on('message', function(message)
      {
        ...
        else if(_options.keepalive && message.cmd === 'keepalive')
        {
          try
          {
